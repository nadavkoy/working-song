...
class ChatPanel(wx.Panel):
    ...
    def get_songs(self):
        while True:
            song = ''
            packets_num = int(self.parent_frame.song_client.client_socket.recv(1024))
            self.parent_frame.song_client.client_socket.send('number of packets received successfully.')
            print 'packets:' + str(packets_num)

            for i in range(packets_num):
                packet = self.parent_frame.song_client.client_socket.recv(1024)
                song += packet
                self.parent_frame.song_client.client_socket.send('packet received successfully.')
            ...
            self.parent_frame.songs_to_play.append(new_file_name)
